...
- 量子振幅エンコーディング(量子状態準備)のQiskit実装(prepare_state)の調査
...
- `debug_dfvqls.py`: テンソル積の検証用デバッグスクリプト
- `test_swap.py`: スワップテストの検証（全テスト通過 ✓）
- `test_tensor_product.py`: テンソル積の形成方法を検証
- `test_kron_ordering.py`: Qiskitのnp.kron順序を検証
...
- 分子回路: 13量子ビット
- 分母回路: 19量子ビット
- 注: 状態準備のボトルネックにより実用的には困難（論文のRemark 2参照）
...
### バグ1: テンソル積の形成（修正済み）
**問題**: 別々の `initialize()` 呼び出しでは正しいテンソル積が作れない
```python
# ❌ 間違い
circ.initialize(psi, [0, 1])
circ.initialize(phi, [2, 3])  # psi ⊗ phi にならない！

# ✓ 正しい
tensor_product = np.kron(psi, phi)
circ.initialize(tensor_product, [0, 1, 2, 3])
```
...
```python
# ❌ 間違い
denominator = norm_K² × |⟨ψ₁|ψ₂⟩|²  # 二乗を使っている

# ✓ 正しい
denominator = norm_K² × √(|⟨ψ₁|ψ₂⟩|²)  # 平方根を取る
```

この修正により、コスト関数が正しい範囲 [0, 1] に収まるようになった。
...
- ✅ テンソル積形成のバグを発見・修正
- ✅ コスト関数の計算式を修正（分母に平方根）
- ✅ 4×4システムで0.015%の精度を達成
- ⚠️ 8×8以上は状態準備のボトルネックで実用困難（論文でも指摘）
